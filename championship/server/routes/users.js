...
router.post('/users', async (req, res) => {
  ...
  const trimmedDisplayName =
    sanitizeString(displayName) || trimmedAccountId;
  const trimmedEmail = sanitizeString(email);
  const normalizedRank =
    normalizedRole === 'player'
      ? sanitizeString(rank) || '언랭크'
      : null;
  ...
  if (!trimmedAccountId || !trimmedPassword) {
    return res
      .status(400)
      .json({ message: 'accountId와 password를 모두 입력해야 합니다.' });
  }

  if (trimmedAccountId.toLowerCase() === 'admin') {
    return res
      .status(403)
      .json({ message: 'admin 계정은 직접 생성할 수 없습니다.' });
  }
  ...
  try {
    ...
    const alreadyExists = users.some(
      (user) =>
        user.accountId &&
        user.accountId.toLowerCase() === trimmedAccountId.toLowerCase(),
    );
    if (alreadyExists) {
      return res
        .status(409)
        .json({ message: '이미 등록된 계정 ID입니다.' });
    }
    ...
    logError('Failed to create user', { message: error.message });
    res.status(500).json({
